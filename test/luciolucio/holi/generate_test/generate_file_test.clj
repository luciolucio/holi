...
(defn run-generate-datelist-test-case [expected holiday-file year bracket-size]
  ...
  (is (= (slurp (format "test-resources/file/%s" expected)) (slurp (format "test-output/%s.datelist" holiday-file)))))
...
(deftest should-generate-weekend-file-when-generate-weekend-datelist!
  ...
  (is (= (slurp "test-resources/file/WEEKEND-EXPECTED.datelist") (slurp "test-output/WEEKEND.datelist"))))
