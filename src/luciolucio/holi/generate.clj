...
(defn -main [bracket-size calendar-file-dir output-path]
  (let [today (t/today)
    ...
    (run! #(file/generate-datelist! (str %) output-path current-year (edn/read-string bracket-size)) files)
    (file/generate-weekend-datelist! output-path current-year (edn/read-string bracket-size))
